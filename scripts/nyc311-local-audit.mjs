...
const service = await SpoolCommandService.create({
  ...
  release: { version: '1.1.0-audit', commitSha: candidateSha }
});
...
try {
  ...
  const approval = await service.approve(request, { expiresAt: '2026-12-31T23:59:59.000Z', nonce: 'nyc311-v1.1.0-production-audit' });
  started = performance.now();
